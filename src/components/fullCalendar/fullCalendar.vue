<template>
  <div class="comp-full-calendar">
    ...
    <fc-header :current-month="currentMonth"
      :first-day="firstDay"
      :locale="locale"
      @change="emitChangeMonth" ref='head'>
      <!-- ref=‘dead’ 获取组件的vue实例 -->
          ...
          <div class="more-body">
            <ul class="body-list">
              <li v-for="(event,index) in selectDay.events"
                  ...
                  @click="eventClick(event, $event)" :key='index'>
                {{event.title}}
              </li>
            </ul>
          </div>
...
<script type="text/babel">
  ...
  export default {
    ...
    mounted () {
      ...
      //new 一个实例对象-------------------------------------
      new swipe(this.$el).left(() => {
        this.$refs.head.goNext();
      }).right(() => {
        this.$refs.head.goPrev();
      })
      //end ----------------------------------
    },
    data () {
